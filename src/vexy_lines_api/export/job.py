...
class JobFolder:
    ...
    def frame_src_path(self, name: str, frame_num: int, ext: str) -> Path:
        """Return ``{job_folder}/{name}--{frame_num}-src.{ext}`` for raw decoded frames."""
        return self._path / f"{name}--{frame_num}-src.{ext}"
    ...
    def existing_src_frames(self, name: str, ext: str) -> set[int]:
        """Scan the job folder for ``{name}--{N}-src.{ext}`` files."""
        pattern = re.compile(rf"^{re.escape(name)}--(\d+)-src\.{re.escape(ext)}$")
        found: set[int] = set()
        if not self._path.exists():
            return found
        for entry in self._path.iterdir():
